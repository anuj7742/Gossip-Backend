...
const getMyChats = TryCatch(async (req, res, next) => {
    ...
    const transformedChats = chats.map(({ _id, name, members, groupChat, lastMessage }) => {
        ...
        .filter(chat => chat !== undefined) // Remove undefined entries
        .sort((a, b) => {
            if (!a.lastMessage && !b.lastMessage) return 0;
            if (!a.lastMessage) return 1; // Place chats with null lastMessage at the end
            if (!b.lastMessage) return -1; // Place chats with null lastMessage at the end
